...
public class Demo012 {
    ...
    private TableRuleConfiguration orderRule(){
        ...
        orderTableRuleConfig.setDatabaseShardingStrategyConfig(
                new StandardShardingStrategyConfiguration("user_id",
                        (names, value) -> "ds" + ((Integer)(value.getValue()) % 2),
                        (names, value) -> {
    ...
    public void selectTest() throws SQLException {
        ...
        try (
            ...
            try (ResultSet rs = preparedStatement.executeQuery()) {
                ShardingResultSet shardingResultSet = (ShardingResultSet) rs;
    ...
    public void selectTest001() throws SQLException {
        ...
        //String sql = "SELECT * FROM t_order o  WHERE (o.user_id=? or o.user_id=?) and other01 > ?";
        //String sql = "SELECT * FROM t_order o  WHERE (o.user_id=? or o.user_id=?) and other01 > ?";
        String sql = "SELECT order_id, count(order_id) FROM t_order group by order_id";
        try (
                Connection conn = dataSource.getConnection();
                PreparedStatement preparedStatement = conn.prepareStatement(sql)) {
            //preparedStatement.setInt(1, 1);
