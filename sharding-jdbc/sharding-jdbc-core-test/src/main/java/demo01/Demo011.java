...
public class Demo011 {
    ...
    public void createOrderItemTable() throws SQLException {
        ...
        Connection conn = dataSource.getConnection();
        execute(conn, "drop table t_order_item", null);
        execute(conn, sql, null);
        conn.close();
    }
    ...
    public void insert() throws SQLException {
        ...
        String orderSql = "insert into t_order(order_id, user_id, status, mark, other01) values(?, ?, ?, ?, ?)";
        String orderItemSql = "insert into t_order_item(order_item_id, order_id, user_id, status, mark, other01) values(?, ?, ?, ?, ?, ?)";

        Connection conn = dataSource.getConnection();

        for(int i=0;i<20;i++){
            List<Object> orderParams = Arrays.asList(i, i%3, "init", "mark:"+i, i%8);
            List<Object> orderItemParams = Arrays.asList(i, i, i%3, "init", "mark:"+i, i);
            execute(conn, orderSql, orderParams);
            execute(conn, orderItemSql, orderItemParams);
        }
    ...
    public void selectTest() throws SQLException {
        ...
        String sql = "SELECT * FROM t_order o  WHERE (o.order_id=? or o.order_id=?) and other01 > ?";
        try (
                Connection conn = dataSource.getConnection();
